...
iosDf = df1['vpn_ios'].unique()
androidDf = df1['vpn_android'].unique()
opers = df1['operator_names'].unique()
df = df[df['operator_names'].isin(opers)]
...
for ik in androidDf:
    if ik != 'Отсутствует':
        newAndroidDf.append(ik)
androidDf = newAndroidDf

newVpnDict = []
for i in vpn_info_dictionary:
    check = False
    for j in iosDf:
        if i['Название на IOS'] == j and "Tachyon VPN - Private Proxy" != j:
            check = True
    for k in androidDf:
        if i['Название на Android'] == k and "Tachyon VPN - Private Proxy" != k:
            check = True
    if check:
        newVpnDict.append(i)
vpn_info_dictionary = newVpnDict
...
def create_table(df, VPN, OS, Oper, Period_begin, Period_end, checkbox2, dfAuto):
    vpn_tmp = []
    f = 0
    for i in VPN:
    ...
    if checkbox2:
        data_for_scatterAuto = dfAuto[(dfAuto['operator_names'].isin(Oper))& (dfAuto['os_names'].isin(OS)) & (dfAuto['sorting_date'].isin(form_full_date_list_df(Period_begin[0], Period_end[0])['Даты']))& ((dfAuto['vpn_android'].isin(VPN_new)) | (dfAuto['vpn_ios'].isin(VPN_new)))] #    
        data_for_scatterAuto['vpn_android'] = data_for_scatterAuto.apply(lambda row: row['vpn_android'] + ' Автоматический', axis = 1)
        data_for_scatterAuto['vpn_ios'] = data_for_scatterAuto.apply(lambda row: row['vpn_ios'] + ' Автоматический', axis = 1)
        VPN_new.append(VPN_new[0] + ' Автоматический')
        data_for_scatter = pd.concat([data_for_scatter, data_for_scatterAuto])
...
def update_k_timeseries(VPN, OS, Oper, Period_begin, Period_end, someCheckbox, someCheckbox2): # City, 
    if someCheckbox != [] and someCheckbox != ['Отобразить по типам сервисов']:
        someCheckbox = []
    if someCheckbox2 != [] and someCheckbox2 != ['Данные по автоматическому тестированию']:
        someCheckbox2 = []
    ...
    tab = create_table(dft, VPN, OS, Oper, Period_begin, Period_end, checkbox2, df1)
    if not checkbox:
